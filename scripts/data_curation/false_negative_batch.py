...
def main():
    ...
    ### Call the batch class on all examples in the dataset
    if args.batch_job:

        ### if filter_query_ids_file is provided, then filter the dataset based on the query_ids
        filter_query_ids = []
        with open(args.query_ids_file, encoding="utf-8") as f:
            for line in f:
                filter_query_ids.append(line.strip())
